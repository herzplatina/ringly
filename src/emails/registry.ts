...
/**
 * Every declared kind must have a template.
 *
 * Typing this as `Record<EmailKind, …>` is what makes that true: adding a kind
 * to EMAIL_KINDS without writing its component fails the build, rather than
 * failing at send time — which, for a suite whose worst-case message is a
 * 48-hour deletion warning, is the difference between a compile error and a
 * business losing its phone number without notice.
 */
export const TEMPLATES: Record<EmailKind, ComponentType<never>> = {
  activation_receipt: billing.ActivationReceipt,
  upcoming_charge: billing.UpcomingCharge,
  payment_succeeded: billing.PaymentSucceeded,
  payment_failed: billing.PaymentFailed,
  payment_reminder: billing.PaymentReminder,
  suspension_notice: billing.SuspensionNotice,
  deletion_warning: billing.DeletionWarning,
  cap_reached: billing.CapReached,
  cancellation_confirmed: billing.CancellationConfirmed,
  calendar_access_failing: operational.CalendarAccessFailing,
  recurring_change: operational.RecurringChange,
  test_calls_exhausted: operational.TestCallsExhausted,
  stats_digest: operational.StatsDigest,
  ops_cap_reached: operator.OpsCapReached,
  ops_payment_failed: operator.OpsPaymentFailed,
  ops_calendar_failing: operator.OpsCalendarFailing,
  ops_activation_stuck: operator.OpsActivationStuck,
  ops_business_deleted: operator.OpsBusinessDeleted,
} as Record<EmailKind, ComponentType<never>>;
