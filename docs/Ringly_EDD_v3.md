...
**No worker settles billing.** Periods open and close because Stripe raised an
invoice, in a webhook handler rather than on a timer ([§2.10.3](#2103-the-rollover-one-webhook-does-the-whole-thing)) —
so a scheduler that stops delays a usage line rather than stopping the money
([R28](#r28)).
...
Stated once so no table below has to repeat them, and so a deviation is visible
as a deviation.

| Convention                                         | Applies to                       | Why                                                                                                                                                         |
| -------------------------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`uuid` primary keys**, generated by the database | Every table but `billing_events` | Ids appear in URLs and in vendor metadata; a sequential integer leaks tenant count and growth rate to anyone who sees one                                   |
| **`bigint` identity** for the ledger               | `billing_events`                 | It is the one table read **in order** ([N10.4](Ringly_PRD_v3.md#n10-4)), and a monotonic key is the ordering                                                |
| **`timestamptz`, never `timestamp`**               | Every instant                    | An instant with no zone is a bug waiting for a DST boundary ([§2.14.1](#2141-timezone-n5))                                                                  |
| **`time` for wall-clock, `date` for local days**   | `business_hours`, rollup days    | "We open at nine" and "Tuesday" are not instants and must survive a DST shift unchanged                                                                     |
| **Money is `integer` cents**                       | Every `*_cents` column           | No floats, no decimals, no currency type; Stripe's API speaks the same unit so nothing converts at the boundary ([§2.10.14](#21014-money-is-integer-cents)) |
| **`text` with a `CHECK`, never a Postgres enum**   | Every closed value set           | Adding a value to a PG enum is a migration that cannot run in a transaction with other DDL; a `CHECK` is an ordinary `ALTER`                                |
| **Nullable `*_at` for "has this happened"**        | Everywhere                       | One idiom for one concept; `IS NULL` reads exactly as `= false` would, at the same price                                                                    |
| **`not null` unless the null means something**     | Everywhere                       | Each nullable column below states what its null means; a null with no stated meaning is a defect                                                            |

**Every tenant table carries `business_id uuid` and leads its primary index with
it** ([§2.3.3](#233-physical-layout)). Foreign keys to `businesses` are `ON DELETE CASCADE` so the
teardown transaction is one `DELETE` ([§2.13.4](#2134-teardown-in-order)); the two exceptions —
`appointments.service_id` and `email_sends.business_id` — are called out where
they appear.
...
businesses
  id                        uuid         pk
  name                      text         -- trading name; enriched from Places, editable before commit
  address                   text         -- one location per business (§1.4)
  timezone                  text         -- IANA name, e.g. 'America/Los_Angeles'; every local boundary uses it
  website                   text null    -- null when the business has none; services came from upload instead
  business_type             text         -- CHECK excludes every healthcare value (§1.4)
  contact_email             citext       -- where all Ringly mail goes, and the Stripe customer's email (F1.10)
  contact_email_verified_at timestamptz null  -- null = unverified; cleared when the address is edited
  google_subject_id         text unique  -- the OAuth `sub`; UNIQUE is one-identity-one-business (§2.5.1)
  phone_number              text null    -- E.164; null until the checklist is green and a number is bought
  telephony_agent_id        text null    -- Retell's agent id, null until provisioned
  agent_bound_at            timestamptz null  -- OBSERVED bind time, written only after read-back agrees (§2.5.3)
  stripe_customer_id        text null    -- created when the contact email is verified
  stripe_subscription_id    text null    -- created when the number goes live; the handle everything hangs off
  service_state             text         -- 'pending' | 'trialing' | 'serving' | 'dormant' (§2.10.1)
  service_started_at        timestamptz null  -- when the number first answered; null while pending
  booking_horizon_days      smallint     -- default 70, CHECK BETWEEN 7 AND 180 (F2.9)
  created_at                timestamptz

billing_events                            -- append-only money ledger; nothing is ever updated
  id                        bigint       pk, identity  -- monotonic: this table is read in order
  business_id               uuid         fk
  kind                      text         -- 'usage_invoiced' | 'invoice_paid' | 'dispute_opened' | …
  amount_cents              integer null -- null for events that carry no money (a dispute opening)
  provider_ref              text null    -- an id Stripe minted; UNIQUE where not null
  idempotency_key           text null    -- a string Ringly minted before Stripe was called; UNIQUE where not null
  period_id                 uuid null    -- fk, null for events not attributable to one period
  occurred_at               timestamptz  -- when the money moved, not when Ringly heard about it

services
  id                        uuid         pk
  business_id               uuid         fk
  name                      text         -- what the agent offers and the caller hears
  description               text null    -- optional; spoken only if the caller asks
  position                  integer      -- display and speaking order; not unique (§2.8.3)
  active                    boolean      -- false = not offered to new callers, existing bookings intact (F3.3)
  deleted_at                timestamptz null  -- soft delete; the row survives so old appointments stay priceable
  created_at                timestamptz

service_versions                          -- only the two attributes with a historical question
  id                        uuid         pk
  service_id                uuid         fk
  price_cents               integer      -- resolved at the APPOINTMENT's start time, not at booking (F3.4)
  duration_minutes          smallint     -- copied onto the appointment at booking; this row is the source
  effective_from            timestamptz  -- a new row per edit; the latest one at or before a moment wins
  created_at                timestamptz

business_hours
  business_id               uuid         fk, pk with weekday
  weekday                   smallint     -- 0 = Sunday … 6 = Saturday
  opens_at                  time         -- WALL CLOCK, not an instant: survives a DST shift unchanged
  closes_at                 time         -- a business closed that day has no row

customers
  id                        uuid         pk
  business_id               uuid         fk
  phone                     text         -- E.164; the customer's identity (F2.4). UNIQUE (business_id, phone)
  name                      text null    -- as spoken; not unique and never used to identify
  created_at                timestamptz

appointments
  id                        uuid         pk
  business_id               uuid         fk
  customer_id               uuid         fk NOT NULL   -- no anonymous bookings (F2.12)
  service_id                uuid         fk NOT NULL ON DELETE RESTRICT  -- see below
  starts_at                 timestamptz  -- the instant; rendered in the business's timezone
  duration_minutes          smallint     -- COPIED at booking and never revisited (F3.4)
  status                    text         -- 'booked' | 'cancelled'
  provider_event_id         text null    -- Google's event id; null only between write and confirmation
  created_at                timestamptz

calls
  id                        uuid         pk
  business_id               uuid         fk
  provider_call_id          text unique  -- Retell's id; the UNIQUE makes a redelivered webhook a no-op
  started_at                timestamptz
  ended_at                  timestamptz null  -- null while the call is in progress
  connected_seconds         integer      -- whole call, not just up to the booking (F6.7)
  is_trial_call             boolean      -- written AT CALL TIME from the snapshot, never derived (F1.12d)
  calendar_incident_id      uuid null    -- fk; set when this call was refused by an outage (F2.7)
  outcome                   text null    -- 'booked' | 'rescheduled' | 'cancelled' | 'enquiry_only' | 'dropped'
  outcome_ruleset_version   integer null -- which classifier ruleset produced it; history is never re-labelled
  classified_at             timestamptz null  -- null = not yet classified; later than the call, which the rollup uses

call_sessions                             -- one conversation's frozen config; not a cache (see below)
  provider_call_id          text         pk
  business_id               uuid         fk   -- the tenancy boundary for a surface with no session
  snapshot                  jsonb        -- catalogue, hours, horizon and was_trialing, as at call start
  opened_at                 timestamptz
  expires_at                timestamptz  -- swept; rows are normally deleted at call end
```
...
deliberate** ([F1.11](Ringly_PRD_v3.md#f1-11)). `contact_email_verified_at` is Ringly's own and lives
...
scheduling_credentials
  business_id               uuid         pk, fk   -- one calendar per business (§1.4)
  provider                  text         -- 'google' today; present from day one so a second needs no backfill
  encrypted_refresh_token   bytea        -- encrypted at rest (N6.1); never logged, never returned by an API
  granted_scopes            text[]       -- what Google ACTUALLY granted, not what was asked (F1.7a)
  connected_at              timestamptz  -- when consent was given
  revoked_at                timestamptz null  -- null = live; set on invalid_grant or a scope that vanished
  last_ok_at                timestamptz null  -- last successful call; how "is this credential usable" is answered

calendar_incidents                        -- one open row per business, max, by partial unique index
  id                        uuid         pk
  business_id               uuid         fk
  opened_at                 timestamptz  -- the first failed read; the email goes on this row's creation
  closed_at                 timestamptz null  -- null = still failing; set by the first successful read
  last_error                text         -- the provider's own message, for the operator not the business
```
...
```
pricing_policy                            -- commercial terms, versioned; rows are never updated
  id                        uuid         pk
  version                   integer unique    -- monotonic; what a period or trial cites
  effective_from            timestamptz  -- new policies are inserted, never edited (F6.16)
  fixed_fee_cents           integer      -- $100 today, charged in advance for a month (F6.1)
  per_minute_rate_cents     integer      -- TBD (Q1); billing cannot go live until it is set
  invoice_cap_cents         integer      -- $500: the ceiling on a PERIODIC invoice, fee included
  usage_cap_cents           integer      -- $400: the ceiling on a FINAL usage invoice, which carries no fee
  billable_outcomes         text[]       -- which `calls.outcome` values are productive (F6.6)
  trial_days                smallint     -- Q7; stated to the business before it commits
  trial_call_allowance      smallint     -- Q7; the bound that makes the trial safe to offer
  retry_attempts            smallint     -- what Stripe's dunning was set to; documentation, not a control
  retry_window_days         smallint     -- CHECK BETWEEN 1 AND 27 — see below; this one is load-bearing

trials                                    -- one bounded free episode per business
  business_id               uuid         pk, fk
  policy_id                 uuid         fk   -- pins the trial to the terms it started under
  started_at                timestamptz  -- when the NUMBER went live, not when the checklist went green
  ends_at                   timestamptz  -- copied from policy.trial_days at creation, so it cannot move
  call_allowance            smallint     -- likewise copied; a policy change cannot shorten a running trial
  calls_used                smallint     -- default 0; incremented atomically, and the increment is the trigger
  ended_at                  timestamptz null  -- null = running
  ended_by                  text null    -- 'days' | 'calls' | 'cancelled'

billing_periods                           -- one row per month a business was served
  id                        uuid         pk
  business_id               uuid         fk
  policy_id                 uuid         fk   -- what it settled under; makes old invoices reproducible
  starts_at                 timestamptz  -- COPIED FROM STRIPE, never computed (see below)
  ends_at                   timestamptz  -- likewise; UNIQUE (business_id, starts_at) makes rollover idempotent
  fee_invoice_ref           text null    -- the Stripe invoice carrying this period's $100
  usage_seconds             integer      -- default 0; billable seconds accrued, the truth before any clamp
  usage_charge_cents        integer null -- what was actually asked for, after the clamp; null until closed
  usage_invoiced_at         timestamptz null  -- null = computed but not yet on an invoice; the next rollover sweeps it
  usage_invoice_ref         text null    -- the Stripe invoice the usage line landed on — not the same as fee_invoice_ref
  closed_at                 timestamptz null  -- null = open; the conditional UPDATE on it settles the same-day race
  closed_by                 text null    -- 'rollover' | 'service_stopped'

usage_records                             -- one row per billable call, ever
  id                        uuid         pk
  business_id               uuid         fk
  period_id                 uuid         fk NOT NULL  -- there is no window with no open period (see below)
  call_id                   uuid         fk UNIQUE    -- one record per call; makes a redelivered webhook free
  connected_seconds         integer      -- summed and rounded up ONCE at period close, never per call (F6.7a)
  created_at                timestamptz  -- a money table: ordering the writes is part of the audit trail
```
...
#### `usage_records.period_id` is `not null`

**There is no window in which Ringly serves a call with no period to attribute it
to.** A period is open from the moment the previous one rolls over until service
stops, and service stopping closes the period it was in and invoices what it
accrued ([§2.10.6](#2106-stopping-service)). A nullable column here could therefore only ever hold
a bug, and `NOT NULL` is what turns that bug into a failed insert.
...
- **`service_state` rather than a billing status**
  ([§2.10.1](#2101-service-state-is-four-values)). Every reader of it wants to know whether the phone
  answers; naming it after money would make each of them infer that from a fact
  about invoices.
- **`is_trial_call` means "inside the free window", not "the owner testing"** —
  a call from a real customer carries it, because the number is live and public
  throughout ([F1.12d](Ringly_PRD_v3.md#f1-12d)). Written at the time of the call, never derived.
- **There is no activation timestamp**, because there is no activation. A
  business starts paying because a trial ended, which `trials.ended_at` records
  along with which bound ended it.
...
```
dormancies                                -- the row's EXISTENCE is the state; there is no 'dormant' flag
  business_id               uuid         pk, fk
  stopped_at                timestamptz  -- the day the phone stopped answering; the clock starts here
  stopped_by                text         -- 'nonpayment' | 'cancelled' — read by the resume guard and nothing else
  due_at                    timestamptz  -- stopped_at + 60 days, moved forward by the length of any pause
  warned_at                 timestamptz null  -- null = the 48-hour warning has not been sent (I4)
  paused_at                 timestamptz null  -- null = running; a paused row is invisible to the sweeper's index
  paused_by                 text null    -- which operator; a pause is never a side effect
  pause_reason              text null    -- free text, shown on the operator queue
  pause_expires_at          timestamptz null  -- default 30 days out; the sweeper resumes the clock by itself

departed_businesses                       -- survives teardown; NO fk, because the business row is gone
  business_id               uuid         pk   -- the id it had, so old references still mean something
  name                      text         -- business identity only: no caller name, number or appointment
  joined_at                 timestamptz
  left_at                   timestamptz  -- the teardown commit, not the day service stopped
  ended_by                  text         -- 'nonpayment' | 'cancelled'
  owed_at_departure_cents   integer      -- read from Stripe AT TEARDOWN, exclusive of tax (F9.9)
  lifetime_net_revenue_cents integer     -- after payment-processor fees; only final at this moment
```
...
so the table needs no `kind` column and does not have one. A deadline keyed by
kind would invite a second: the moment two rows can exist for one business,
every query has to say which it means and every transition has to say which it
clears.

The sweeper's queries are the whole of the lifecycle:
...
```
daily_call_rollups                        -- what every dashboard figure but the median is served from
  business_id               uuid         fk, pk with day
  day                       date         -- the business's LOCAL day; two timezones key the same date differently
  calls                     integer      -- total calls that day, whatever the outcome
  connected_seconds_sum     bigint       -- total connected time; average duration is this ÷ calls
  appointments_booked       integer      -- the headline tile; a call books at most one (F5.3)
  revenue_booked_cents      integer      -- an ESTIMATE where the range includes future appointments (F3.4)
  counts_by_outcome_window  jsonb        -- 5 outcomes × 6 four-hour windows; one structure serves both charts
  computed_at               timestamptz  -- compared against calls.classified_at to find days needing recompute

cost_records                              -- what Ringly SPENT, including on calls it charged nothing for
  id                        uuid         pk
  business_id               uuid null    -- fk; NULL only for 'enrichment', which precedes the tenant (N9.2)
  day                       date
  source                    text         -- 'telephony' | 'classifier' | 'enrichment'
  amount_cents              integer      -- recorded in full even past the cap, or the subsidy is invisible
```

**There is exactly one duration column, and it is not billable time.**
`connected_seconds_sum` covers **every** call the business took; billable seconds
live in `usage_records` ([§2.4](#24-data-model)/007) and count only productive ones ([F6.6](Ringly_PRD_v3.md#f6-6)). Two
duration columns on one rollup row is how a dashboard comes to show two different
averages, so there is one — and the figure a business is charged on is
...
```
email_sends                               -- the queue, the outbox and the delivery record, in one row
  id                        uuid         pk
  reason_key                text unique  -- "is there a REASON to send this at all"; per period, incident or event
  business_id               uuid         -- deliberately NO fk: the row must outlive the business (see below)
  kind                      text         -- the registry's closed set (§2.11.1); nothing outside it is ever sent
  to_address                citext       -- resolved at enqueue, so a later address edit cannot redirect it
  identity                  text         -- 'billing' | 'service' | 'reports' | 'operator' — four sending subdomains
  subject                   text         -- RENDERED AT ENQUEUE, not at send (see below)
  body                      text         -- likewise; it must survive its own subject being deleted
  queued_at                 timestamptz
  claimed_at                timestamptz null  -- null = unclaimed; the claim is what makes one worker own the row
  sent_at                   timestamptz null  -- null = not yet handed to the provider successfully
  attempts                  smallint     -- default 0; drives the ≈14¾-hour backoff ladder
  last_error                text null     -- the most recent failure, for the operator queue
  provider_idempotency_key  text         -- sent as Resend's header; ≤256 chars, 24-hour window
  provider_message_id       text null    -- Resend's id, for correlating the bounce webhook back
  bounced_at                timestamptz null  -- set by email.bounced: accepted by Resend, refused by the recipient
```
...
**Views and indexes only; no new tenant data, and no columns.** Per-business revenue, cost and
...
([F1.11](Ringly_PRD_v3.md#f1-11)), and a provider write that reports success without taking effect
([F1.11c](Ringly_PRD_v3.md#f1-11c)).
...
7. **The checklist** ([F1.11](Ringly_PRD_v3.md#f1-11)) — three tasks in any order, with the trial's two
   bounds stated alongside so the business knows what it is agreeing to before it
   finishes ([F1.12a](Ringly_PRD_v3.md#f1-12a)).
...
9. **The trial starts by itself** ([F1.11a](Ringly_PRD_v3.md#f1-11a)). There is no ninth thing for the
...
business without a connected calendar cannot take bookings, and [F1.11](Ringly_PRD_v3.md#f1-11) makes a
...
**There is no fourth item asking whether the agent sounded right** ([F1.12e](Ringly_PRD_v3.md#f1-12e)).
It would gate nothing, and the signal it would produce is derived from calls and
bookings instead ([§2.5.4.1](#2541-the-failing-trial-signal)).

**Editing the contact address clears its verification**, because [F1.10](Ringly_PRD_v3.md#f1-10) exists to
...
decline notices Stripe sends reach the same inbox ([F1.10](Ringly_PRD_v3.md#f1-10)).
...
verification either way** — [F1.10](Ringly_PRD_v3.md#f1-10) exists to stop the 48-hour deletion warning
...
([F1.11](Ringly_PRD_v3.md#f1-11)) with three different owners — a verified contact email (Ringly's), a
...
([F1.11b](Ringly_PRD_v3.md#f1-11b)). Steps 1 and 2 depend on a third party and can take minutes or,
...
which is why this subsection is short.
...
[F1.11c](Ringly_PRD_v3.md#f1-11c), and it is the one piece of vendor interaction the design does not
...
make; [F1.11c](Ringly_PRD_v3.md#f1-11c) says a failed verification is retried, and this says by whom.
...
read-back written against a stale shape would find `undefined`, compare it to
...
| ---------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| **Bind**   | Provisioning stuck ([F1.11c](Ringly_PRD_v3.md#f1-11c), [F8.6](Ringly_PRD_v3.md#f8-6)) | [F8.12](Ringly_PRD_v3.md#f8-12), "Provisioning stuck"  |
...
**It is a read of provider state, never a placed call** ([F1.11c](Ringly_PRD_v3.md#f1-11c)). A synthetic
...
[F1.11](Ringly_PRD_v3.md#f1-11)'s checklist item 2 exists for exactly that.

### 2.5.4 The trial's two bounds

The trial ends at whichever bound is reached first ([F1.12a](Ringly_PRD_v3.md#f1-12a)), and the two are
...
**Neither bound unbinds the agent** ([F1.12b](Ringly_PRD_v3.md#f1-12b)). The number keeps answering and
billing simply begins. **`stopService` is not called from anywhere in the trial
path**, and the only unbind in the product is the one that ends service for good
([§2.10.6](#2106-stopping-service)) — stated in the negative because "the trial ended" and "service
stopped" are the two events most easily conflated.
...
of the fixed fee ([F1.12](Ringly_PRD_v3.md#f1-12)), and the cleanest way to guarantee the first invoice
...
**The trial is the whole product** ([F1.12](Ringly_PRD_v3.md#f1-12)), so there is no branch anywhere in
...
**It is derived rather than asked** ([F1.12e](Ringly_PRD_v3.md#f1-12e)). Asking depends on the business
telling Ringly, and the business having the worst time is the least likely to say
anything.
...
| #   | Decision                                                                                              | Because                                                                                                                                                                                                     |
| --- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Commit the business row **before** checking granted scopes                                            | [F1.7b](Ringly_PRD_v3.md#f1-7b)'s "the draft is kept" is only durable if it is a row                                                                                                                        |
...
**The obvious optimisation — caching a `hasOpenCalendarIncident` flag to skip the
no-op `UPDATE` on healthy calls — is not available, and it fails twice over.**
...
**Reported by calendar month, not by each business's own billing month** ([F8.8](Ringly_PRD_v3.md#f8-8)). No
...
`businesses.service_state` is the one column the call path, the sweeper and the
bind reconciler all read. **It names the phone, not the money**, because every
caller of it wants to know whether the number answers — and a column named after
billing would make each of them infer that from a fact about invoices.

| State      | The agent is | Meaning                                                                 |
| ---------- | ------------ | ----------------------------------------------------------------------- |
| `pending`  | unbound      | Checklist incomplete, or the number is still being provisioned          |
| `trialing` | **bound**    | Full service, free, inside the trial ([F1.12](Ringly_PRD_v3.md#f1-12))  |
...
subscription buys: an activation charge outside the invoice sequence would mean
every property that holds for ordinary periods — tax, receipts, retries, the
clamp — has to be re-argued for period 1.
...
**Nothing happens**, and this section exists to say so — it is the place a
reader most expects to find machinery.
...
Ringly has decided to stop metering ([F1.11c](Ringly_PRD_v3.md#f1-11c)). Pausing the subscription
...
**It is also why the daily reconciliation is credible** ([F6.11d](Ringly_PRD_v3.md#f6-11d)): the
...
([F6.10a](Ringly_PRD_v3.md#f6-10a)), and Stripe raises the new period's invoice immediately. The old
...
**There is no revocation endpoint**, because there is no window to revoke inside
([F6.12](Ringly_PRD_v3.md#f6-12)). A business that changes its mind resumes ([§2.10.10](#21010-coming-back)), which
restores its number, its history and its subscription.
...
number is confirmed live** ([F1.11a](Ringly_PRD_v3.md#f1-11a)) — so `trial_end` and `trials.started_at`
...
single time**, and it looks like success at every point except the inbox. [F1.10](Ringly_PRD_v3.md#f1-10)
...
There is deliberately no way to delete one ([F9.1a](Ringly_PRD_v3.md#f9-1a)), and the absence has
teeth:
...
There is no path on which that is false, which is what makes it a rule rather
than a rule with an exception ([§2.4](#24-data-model)/008).
...
This is not a formatting concern. A four-hour analytics window, a local day
boundary and an opening-hours check are all boundary computations, and getting any
of them in UTC gives a business in Los Angeles figures that are wrong for a third
of every day.

**Three storage choices follow from it and are not interchangeable:**

| What                               | Stored as     | Because                                                                               |
| ---------------------------------- | ------------- | ------------------------------------------------------------------------------------- |
| An appointment, a call, an invoice | `timestamptz` | It is a moment; the timezone is presentation                                          |
| Opening hours                      | `time`        | "We open at nine" is a fact about the clock, and stays true across a DST shift        |
| A rollup's day                     | `date`        | It is a local calendar day, and has no instant of its own ([§2.9.2](#292-the-rollup)) |

**The rollup's `date` is the subtle one.** Two businesses in different timezones
produce rows keyed on the same `date` from different UTC ranges, and that is
correct: each is asking about its own day. Storing an instant instead would make
the key meaningless across tenants.
...
- Opening hours survive a DST transition unchanged: a business that opens at nine
  opens at nine on both sides of it.
...
- An invoice dated by the provider is rendered in the business's timezone and
...
- <a id="q7"></a>**Q7 — the trial's two bounds** ([F1.12a](Ringly_PRD_v3.md#f1-12a)). Configuration, so neither blocks
  building the trial — but they are shown to the business before it commits
  ([F1.11](Ringly_PRD_v3.md#f1-11)), so a placeholder is visible to a customer in a way a placeholder
...
- <a id="r33"></a>**R33 — The trial gives away full service, and the call bound is the only
  ...
  calendar at Ringly's cost ([F1.12](Ringly_PRD_v3.md#f1-12)), and the day bound does nothing to limit it.
...
- <a id="r34"></a>**R34 — Three vendors can each take the phone off the air, and only one of
  them tells Ringly.** A call needs Retell to route it, Google to verify a slot,
  and — for anything that follows — Stripe to be reachable. The design fails
  closed on Google ([§2.6.4](#264-fail-closed-concretely)) and detects Retell drift by reading provider
  state back ([§2.5.3](#253-bind-and-unbind-are-verified-by-reading-provider-state-back)), but **there is no synthetic check that the number
  answers**, because dialling it costs minutes on every probe and proves only
  that something picked up ([F1.11c](Ringly_PRD_v3.md#f1-11c)).
  - **The residual is a Retell-side failure that leaves the binding intact**: the
    read-back agrees, no incident opens, and the first party to notice is a
    caller who cannot get through.
  - **Partly covered by absence**: a business with a live number and no calls for
    an unusual number of days is visible on the operator's own figures, and a
    trial with no calls at all is already surfaced ([§2.5.4.1](#2541-the-failing-trial-signal)).
  - **Not mitigated further in v3.** The honest fix is an out-of-band synthetic
    call on a schedule, and it is [A1](Ringly_PRD_v3.md#a1)'s manual QA until there is evidence it is
    needed.

- <a id="r35"></a>**R35 — The rollup is a second source of truth for figures a business acts
  on.** Every dashboard number except the median and the money comes from
  `daily_call_rollups` rather than from `calls` ([§2.9.2](#292-the-rollup)), so a bug in the
  rollup shows a business figures its own call history does not support — and
  **nothing compares the two**.
  - **The recompute path narrows it**: any day holding a late-classified call is
    rebuilt, so a systematic classification lag self-corrects.
  - **It does not cover an arithmetic error in the rollup itself**, which would be
    consistent, plausible and wrong in every row.
  - **Mitigated by making the rollup derivable**: it is a pure function of one
    business's calls for one day, so a discrepancy check is a query rather than a
    project. **Not scheduled in v3** — deferred until there is production data to
    check it against, and recorded here so it is a decision rather than an
    oversight.

- <a id="r36"></a>**R36 — A Google refresh token can expire under a business that is doing
  nothing wrong.** Credentials are revoked by the user, by Google after prolonged
  inactivity, or by a password change; a dormant business ([§2.13.1](#2131-one-clock-and-the-sweeper-that-runs-it)) is by
  definition inactive for up to 60 days, and an operator-paused one for longer
  still.
  - **On resume this surfaces as bookings failing rather than as a resume
    failure**, because `resume()` rebinds the agent and does not exercise the
    calendar ([§2.10.10](#21010-coming-back)).
  - **Recommendation, and the reason it is a risk rather than a mechanism**: a
    calendar probe on resume would catch it at the right moment, and it is one
    API call on a path that runs rarely. It is not in [§2.10.10](#21010-coming-back) because a
    failing probe must not block a business getting its number back — so it would
    be an alert and a banner, which is [§2.6.4](#264-fail-closed-concretely)'s incident machinery reached
    by a second route. **Left as a risk pending a decision** on whether resume
    opens an incident or merely reports one.
...
- <a id="r23"></a>**R23 — A business cannot action a consumer erasure request through Ringly.**
  There is no per-customer deletion ([F9.1a](Ringly_PRD_v3.md#f9-1a)), so the only way to remove one
...
- <a id="r25"></a>**R25 — A silent unbind failure leaks revenue with no other symptom.** Every
  ...
  would ever notice ([F1.11c](Ringly_PRD_v3.md#f1-11c)). Mitigated by reading provider state back on
...
**To be derived once these two documents are locked**, from the **Behaviours owed
to the catalogue** lists at the end of [§2.2](#22-architecture)–[§2.14](#214-cross-cutting-properties) — which is why
those lists exist and why each section is required to produce one.

Each scenario **names the requirement it holds**, belongs to exactly one group,
and is written as something a person does and what then becomes true. The
catalogue is therefore also the coverage map: a requirement with no scenario
against it is visible as a gap in one place rather than inferred from an absence
across twenty.

**It is deliberately last.** A catalogue written against requirements that are
still moving is a catalogue that has to be rewritten, and the scenarios are
written in the vocabulary of the design sections above — which is settled only
when they are.
